...
Comprehensive investigation and technical post-mortem of the intermittent incoming audio cutout bug in Mumla OLED, the underlying Android `AudioTrack` pacing mechanics, historical origins across versions, incidental cryptographic findings, and the permanent architectural resolution deployed in v0.21.5.
...
1. In v0.20.5, render-lead pacing was introduced in `AudioOutput.run()` to prevent the render loop from sprinting ahead of network packets and draining the native jitter buffer on burst starts.
2. The loop calculated the unplayed buffer lead:
   ```math
   \text{lead} = \text{writtenTotal} + \text{RENDER\_SAMPLES} - \text{played}
   ```
   where $\text{played} = \text{AudioTrack.getPlaybackHeadPosition()}$. If $\text{lead} > \text{maxLeadSamples}$, the loop paused for 5 ms, waiting for the hardware playback head to advance.
...
6. Because the render loop deadlocked in its pacing while-loop, `AudioOutputEngine::renderMix` was never invoked again. In Mumla, user talk state transitions (`TalkState.TALKING`, `TalkState.PASSIVE`) are evaluated and dispatched exclusively during the render pump. Consequently, UI talking indicators froze passive, and incoming voice packets queued up unread in native memory.
...
sequenceDiagram
    ...
    participant Engine as NativeAudioOutputEngine (C++)
    ...
    loop Every 20 ms Quantum
        Java->>Java: Pacing Check: lead <= maxLeadSamples?
...
2. **Native Output Engine (`AudioOutputEngine.{h,cpp}`)**:
   Maintains a per-user Speex adaptive jitter buffer and Opus decoder. It mixes multi-speaker streams, applies soft saturation limiting, and detects user talk state transitions (`TALKING`, `SHOUTING`, `WHISPERING`, `PASSIVE`).
3. **Render Pump (`AudioOutput.java`)**:
   A dedicated Java thread running at `THREAD_PRIORITY_URGENT_AUDIO`. It pulls 20 ms quanta (960 samples at 48 kHz) from native code and writes them into `android.media.AudioTrack` in `MODE_STREAM`.

---

## 3. Historical Timeline Across Releases

The investigation revealed that this bug was not part of the original legacy Mumla codebase, nor was it introduced during the native C++ engine rewrite in v0.20.0. It was introduced in v0.20.5 as a regression while solving a different audio defect.

| Mumla Version | Output Pipeline Architecture | Pacing Mechanism | Vulnerable to Pacing Deadlock? |
|---|---|---|---|
| **$\le$ 0.19.0** | Legacy Java mixer (`BasicClippingShortMixer`) | Blocking `AudioTrack.write()` only; explicit `pause()`/`flush()` on silence | **No** (Never queried `getPlaybackHeadPosition()`) |
...
### The Genesis in v0.20.5 (Commit `b1e753b1`)
...
5. Once the last sample plays, the hardware buffer is empty. At this moment:
   ```math
   \text{writtenTotal} = \text{played} \quad \implies \quad \text{lead} = 0
   ```
6. Android's `AudioFlinger` detects an empty buffer on an active track and logs:
   ...
   ```
7. When Speaker B talks 3 seconds later:
   ```math
   \text{lead} = 0 + 960 - 0 = 960 \le \text{maxLeadSamples}
   ```
   The pacing check passes immediately on the first poll. Audio flows seamlessly.
...
5. When the next packet arrived:
   ```math
   \text{lead} = 2400 + 960 - 600 = 2760
   ```
...
In Mumla, user talk state changes are handled in `AudioOutputEngine.cpp`:
```cpp
size_t AudioOutputEngine::renderMix(int16_t* out, size_t samples) {
    ...
    // Collect talk-state transitions for active users
    if (m_talkListener) {
        for (const auto& ev : talkEvents) {
            m_talkListener->onTalkStateChanged(ev.session, ev.state);
        }
    }
}
```
...
### 1. Latent CryptState OCB2 Replay Check Typo (Present since 2014)
...
```
In 2014 (commit `97634aea`), during a refactor of the Jumble crypto layer, `mDecryptIV[1]` was accidentally typed as `mEncryptIV[0]`. If the low byte of the local encryption IV happened to match the decrypt history byte, valid incoming out-of-order voice packets were falsely dropped as replay attacks.
...
stateDiagram-v2
    ...
    note right of BurstStart: wasIdle == true\nRebase: writtenTotal = played\nReturn Action.PROCEED

    BurstStart --> Rendering: Pacing Check Passed
    Rendering --> Rendering: AudioTrack.write()\nwrittenTotal += written

    Rendering --> PacingWait: lead > maxLeadSamples
    PacingWait --> Rendering: Hardware Consumes Frames (lead <= maxLeadSamples)
...
- **`AudioOutputPacerTest.java`**:
  - `testPacerSizesLeadBoundToTrackBuffer()`: Verifies hardware buffer floor and expansion.
  - `testPacerIdleRebaseEliminatesLag()`: Verifies zero-lag resumption after silence.
  - `testPacerStallBreakoutAfter40ms()`: Verifies exact 8-poll (40 ms) breakout timing.
  - `testPacerSubsequentStallRequiresFullDuration()`: Verifies stall counter reset to prevent unpaced runaway.
  - `testPacerNonZeroHeadStallBreakout()`: Verifies breakout at arbitrary counter offsets.
  - `testPacerWraps32BitHeadGracefully()`: Verifies 32-bit hardware overflow (~24.9 hours).
  - `testPacerRebasesOnOemSpuriousReset()`: Verifies recovery from OEM driver downward resets.
- **`CryptStateTest.java`**:
  - `testOcb2ReplayCheckMatchesUpstream()`: Validates out-of-order packet acceptance against Mumble parity.
  - `testOcb2SignedByteLostPacketCalculation()`: Validates packet loss calculations above IV 127.

### 2. Full Project Gate (`./scripts/check.sh`)
- 59 Python repository and format tests passed.
- 79 Native C++ audio engine tests passed.
- 57 Gradle unit tests passed.
