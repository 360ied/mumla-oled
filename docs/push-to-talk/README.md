...
| **PTT-14** | **Audio Feedback** | **Low** | **Resolved** | **Muted PTT sound on disabled system clicks & missing release cue**: Replaced global OS keypress sound with dedicated `SoundPool` on `STREAM_MUSIC`/`STREAM_VOICE_CALL`, loading upstream Mumble radio chirps (`ptt_on.ogg`, `ptt_off.ogg`, 3-Clause BSD) on both activation and deactivation. | [`MumlaService.java:465`](file:///home/bualy/files/devel/mumla_dev/mumla-oled/app/src/main/java/se/lublin/mumla/service/MumlaService.java#L465-L530) |
