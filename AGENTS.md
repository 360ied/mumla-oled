...
- **Documentation Exception**: Standalone documentation changes (such as editing `AGENTS.md`, `README.md`, standalone documentation files under `docs/`, adding or updating agent skills under `.agents/skills/`, or editing agent guidelines) do not need the worktree process and should be made directly on `master`. Documentation that directly accompanies active feature or bugfix code development should remain in the corresponding feature worktree.
